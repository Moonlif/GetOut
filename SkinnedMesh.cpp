...
{
	effect = LoadEffect("./player/MultiAnimation.hpp");
...
void SkinnedMesh::UpdateAndRender()
{
	...
	if (rootFrame)
	{
		D3DXMATRIXA16 world, scaleMat, rotY, translatrionMat;
		D3DXMatrixIdentity(&world);

		D3DXMatrixScaling(&scaleMat, size, size, size);
		D3DXMatrixRotationY(&rotY, rotationAngle);
		...
		D3DXMatrixTranslation(&translatrionMat, position.x, position.y, position.z);
...
void SkinnedMesh::Destroy()
{
	...
	SAFE_DELETE_ARRAY(workingPalette);
	SAFE_RELEASE(effect);
}

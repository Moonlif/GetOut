...
void cObjLoader::LoadMtlLib(char * szFolder, char * szFile, int blank)
{
	std::string sFullPath(szFolder);
	sFullPath += (std::string("/") + std::string(szFile));
...
LPD3DXMESH cObjLoader::LoadMesh(OUT std::vector<cMtlTex*> & vecMtlTex, IN char* szFolder, IN char* szFile, int blank)
{
	std::vector<D3DXVECTOR3> vecV;
	std::vector<D3DXVECTOR2> vecVT;
	std::vector<D3DXVECTOR3> vecVN;
	...
	while (true)
	{
		...
		if (szTemp[0] == 'm')
		{
			char szMtlFile[1024];
			sscanf_s(szTemp, "%*s %s", szMtlFile, 1024);
			LoadMtlLib(szFolder, szMtlFile, blank);
		}
		else if (szTemp[0] == 'g')
		{

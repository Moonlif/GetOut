...
void cInteract::Update()
{
	...
	if (g_pData->m_bStuffSwitch[SWITCH_SECONDFLOOR_BUTTON1] && m_vecStuff[STUFF_BUTTON1]->GetSwitch() == false)
		m_vecStuff[STUFF_BUTTON1]->Reposition(D3DXVECTOR3(-9.5f, 24, 14), D3DXVECTOR3(D3DX_PI, 0, 0));
	if (g_pData->m_bStuffSwitch[SWITCH_SECONDFLOOR_BUTTON1] == false && m_vecStuff[STUFF_BUTTON1]->GetSwitch() == false)
		m_vecStuff[STUFF_BUTTON1]->Reposition(D3DXVECTOR3(-9.5f, 24.5f, 14), D3DXVECTOR3(D3DX_PI, 0, 0));
	...
	if(m_n2FButtonCount > 5) g_pData->m_bStuffSwitch[SWITCH_SECONDFLOOR_BUTTON1] = true;
	else g_pData->m_bStuffSwitch[SWITCH_SECONDFLOOR_BUTTON1] = false;

	//picking
	int keyState = 0;
	if (GetAsyncKeyState(VK_LBUTTON) & 0x0001 && g_pData->GetIsInvenOpen() == false) keyState = 1;
	else if (GetAsyncKeyState(VK_RBUTTON) & 0x0001 && g_pData->GetIsInvenOpen() == false) keyState = 2;
